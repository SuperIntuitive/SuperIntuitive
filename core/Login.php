...
class Login {
	...
	public function Attempt($post){
		if(isset($post['email']) && isset($post['password'])){
			...
			//$this->Logout();
			$_SESSION['SI']['domains'][SI_DOMAIN_NAME]['businessunits'][SI_BUSINESSUNIT_NAME]['AJAXRETURN']['LOGINFAIL'] = 'TRUE';
	...
	private function Verified($ouruser, $post){
		...
		$usersession = $_SESSION['SI']['domains'][SI_DOMAIN_NAME]['businessunits'][SI_BUSINESSUNIT_NAME]['user'];
		...
		if(count($roles)==0){
			...
			foreach($roles as $role){
				...
				if(!in_array($role['name'], $_SESSION['SI']['domains'][SI_DOMAIN_NAME]['businessunits'][SI_BUSINESSUNIT_NAME]['user']['roles'])){
					$_SESSION['SI']['domains'][SI_DOMAIN_NAME]['businessunits'][SI_BUSINESSUNIT_NAME]['user']['roles'][] = $role['name'];
					$_SESSION['SI']['domains'][SI_DOMAIN_NAME]['businessunits'][SI_BUSINESSUNIT_NAME]['user']['permissions'] = json_decode($role['rules'],true);
				}
			}
		}
	...
	public function Logout(){
		if(isset($_SESSION['SI']['domains'][SI_DOMAIN_NAME]['businessunits'][SI_BUSINESSUNIT_NAME]['user']['id'])){
			...
			setcookie('remembermetime', null, -1, '/'); 
			$rememberuser = new Entity('users');
			$rememberuser->Id = $_SESSION['SI']['domains'][SI_DOMAIN_NAME]['businessunits'][SI_BUSINESSUNIT_NAME]['user']['id'];
			$rememberuser->Attributes->Add(new Attribute('remembertoken', ''));
			$rememberuser->Attributes->Add(new Attribute('remembertime', 'null'));
			$rememberuser->Update();
		}
	...
	public function ChangePassword($post){
		if ($this->Verify() && isset($post['newpassword']) && (!empty($_SESSION['SI']['domains'][SI_DOMAIN_NAME]['businessunits'][SI_BUSINESSUNIT_NAME]['user']['id'] ))) {	
			$user = new Entity("users");
			$user->Id = $_SESSION['SI']['domains'][SI_DOMAIN_NAME]['businessunits'][SI_BUSINESSUNIT_NAME]['user']['id'];
